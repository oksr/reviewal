...
- **`ui/theme.rs`** — semantic color roles (`Theme`), built once via `Theme::load(&config)` → `(Theme, warnings)`; persona colors resolve on demand with `theme.persona_color(name, frontmatter_color)` (builtin slots, then FNV-1a hash into the accent-filtered pool). `NO_COLOR` ⇒ monochrome. `[theme]` config keys merge per-field, never whole-table.

Other pieces: `config.rs` layers project `.reviewal/config.toml` over global `~/.config/reviewal/config.toml` (XDG-aware), and `config::load` is the *only* place ambient environment is read — it records the global persona dir on `Config`, and everything downstream (UI, engine, CLI) resolves persona directories through `Config::persona_dirs`, never env. This is a hermeticity invariant: a `Config::default()` has no global dir, so tests can't be broken by whatever lives in the developer's real `~/.config/reviewal/personas/`. `skill.rs` implements `reviewal init`, which installs `assets/SKILL.md` (embedded via `include_str!`, version-stamped with `reviewal-version:` for upgrade detection) into `.claude/skills/reviewal-ingest/`.
